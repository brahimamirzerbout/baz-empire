...
How Nomad List dominates longer tail keywords
is a crowdsourced database of cities for Digital Nomads. You can filter the cities by hundreds of categories: air quality, cost of living, etc.
As you apply filters the database is queried and the home page dynamically updates to display the best matching city:
Crucially, the results of each filter query are also
. Autogenerated pages kitted out with meta and h1 tags, etc …
...
picking up traffic from that solitary keyword. It ranks for well over a hundred similar keywords such as “top ten racist states”, “least racist city” etc…
...
Interestingly, the majority of Nomad List's organic traffic comes from filtering by
. And then targeting the keyword phrase “best cities to live in [region]“ with meta and h1 tags:
...
Contrast this with a 99% of other companies who have no option but to research and write blog posts
to compete with Nomad List for the same longer tail keywords.
How would I replicate this?
Essentially, all that's going on is your just bundling useful content under unqiue url's. With this very website for instance, every single filter (as well as being used to query the home page) is also
unique page as well. For example:
